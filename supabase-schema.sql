...
DROP POLICY IF EXISTS "profiles_insert_nonadmin" ON public.profiles;
...
-- Чтение и обновление профилей — открыто (для простоты админки).
-- НО: новую запись и любые изменения можно делать только с is_admin = 0,
-- иначе любой смог бы зарегистрироваться или назначить себя админом.
CREATE POLICY "profiles_public_all"      ON public.profiles      FOR SELECT USING (true);
CREATE POLICY "profiles_public_all"      ON public.profiles      FOR UPDATE USING (true) WITH CHECK (is_admin = 0);
CREATE POLICY "profiles_insert_nonadmin" ON public.profiles      FOR INSERT WITH CHECK (is_admin = 0);
...
-- ============================================================
-- RPC: активация ключа из луа (HTTP POST /rest/v1/rpc/activate_key)
-- Принимает код ключа -> выдаёт срок (по МСК) -> помечает ключ
-- использованным. Возвращает license_end (unix, секунды по МСК).
-- ============================================================
...
DECLARE
    ...
    v_now BIGINT;
BEGIN
    v_now := (EXTRACT(EPOCH FROM now()))::BIGINT + 3 * 3600; -- МСК

    SELECT * INTO v_key
    FROM public.license_keys
    WHERE key = p_key AND (used_by IS NULL OR used_by = '')
    LIMIT 1;
    ...
    SET used_by = 'lua_webhook', used_at = now()
    WHERE key = p_key;

    RETURN v_now + v_key.days * 86400;
END;
$$;

GRANT EXECUTE ON FUNCTION public.activate_key(TEXT) TO anon, authenticated;